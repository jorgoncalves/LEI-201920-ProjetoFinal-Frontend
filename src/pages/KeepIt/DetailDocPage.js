...
import { getRecs } from '../../util/restCall_records';
...
export default function DetailDocPage(props) {
  ...
  const getRecords = async () => {
    ...
    if (resp.status !== 500) {
      if (resp.data) setRecord(resp.data.records);
    } else {
  ...
  const sortTags = async (el) => {
    ...
    if (sortItem < 0) {
      ...
      for (var [key, valueRec] of Object.entries(records)) {
        if (valueRec) {
          let tempValue =
            valueRec.tags[sortItem] === undefined ||
            valueRec.tags[sortItem] === ''
              ? '#'
              : valueRec.tags[sortItem];
          var Indexing = arrayTemp.findIndex(
            (x) => Object.entries(x)[0][0] === tempValue
          );
          if (Indexing < 0) {
            arrayTemp.push(JSON.parse(`{"${tempValue}":[]}`));
            await arrayTemp[
              arrayTemp.findIndex((x) => Object.entries(x)[0][0] === tempValue)
            ][tempValue].push(JSON.parse(JSON.stringify(valueRec)));
          } else {
            await arrayTemp[Indexing][tempValue].push(
              JSON.parse(JSON.stringify(valueRec))
            );
          }
        }
        ...
        <>
          ...
          <div className="submitBox">
            <h2 className="uk-heading-divider uk-margin-medium-bottom">
              {documentInfo.name}
              {documentInfo.status === 'obsolete' ? (
                <span className="docStatus uk-margin-small-left">
                  (Obsolete)
                </span>
              ) : (
                ''
                ...
                <div className="leftBox">
                  ...
                  {documentInfo.isModelFile && (
                    <Icon
                      icon="download"
                      tooltip="Download"
                      link={`${geFile}?path=${documentInfo.path}`}
                    />
                  )}
                </div>
                ...
                >
                  {recordsLoading ? (
                    ...
                    sortedRecords.length > 0 ? (
                      ...
                      <h4 className="uk-text-center uk-text-bold uk-margin-medium-top">
                        THERE ARE NO RECORDS
                      </h4>
                    )
                  ) : records.length > 0 ? (
                    records.map((rec, i) => {
                      return <RecsShow key={i} record={rec} />;
                    })
                  ) : (
                    <h4 className="uk-text-center uk-text-bold uk-margin-medium-top">
                      THERE ARE NO RECORDS
